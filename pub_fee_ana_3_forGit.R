...
# df_NAclean <- df_combine %>% filter(!is.na(Price))
ggplot(df_NAclean, aes(x = year, y = log10(ratePrice), group = journal, color = journal)) + geom_line(size=2, alpha=0.5) + # geom_point() +
  scale_color_manual(values = cols) +
  theme_minimal() +
  labs(title = "", x = "Year", y = "USD") +
  xlim(NA, 2024) +
  ylim(0, 12000) +
  # add label of journal name
  geom_text(data = df_NAclean %>% filter(year == 2023), aes(label = journal), hjust = +0.9, vjust = 0, size = 8) +
  # change legend title
  labs(color = "") +
  # change font size of x axis title and legend
...
ggplot(df_NAclean, aes(x = year, y = log10(ratePrice), group = journal, color = journal)) + geom_line(size=1) + # geom_point() +
  scale_color_manual(values = cols) +
  theme_minimal() +
  labs(title = "Price", x = "Year", y = "Rate change in fees") +
  xlim(NA, 2023) +
  # add label of journal name
  geom_text(data = df_NAclean %>% filter(year == 2023), aes(label = journal), hjust = +0.9, vjust = 0, size = 3)
...
ggplot(df_NAclean, aes(x = year, y = log10(rateImpFac), group = journal, color = journal)) + geom_line(size=1) + # geom_point() +
  scale_color_manual(values = cols) +
  theme_minimal() +
  labs(title = "Impact Factor", x = "Year", y = "Rate change in Impact Factor") +
  xlim(NA, 2022) +
  # add label of journal name
  geom_text(data = df_NAclean %>% filter(year == 2022), aes(label = journal), hjust = +0.9, vjust = 0)
...
ggplot(df_NAclean, aes(x = year, y = log10(ratePubNum), group = journal, color = journal)) + geom_line(size=1) + # geom_point() +
  scale_color_manual(values = cols) +
  theme_minimal() +
  labs(title = "Publication counts", x = "Year", y = "Rate change in counts") +
  xlim(NA, 2023) +
  # add label of journal name
  geom_text(data = df_NAclean %>% filter(year == 2023), aes(label = journal), hjust = +0.9, vjust = 0)
...
# Draw line graph for CHN

# Remove NA in PubNum
df_NAclean <- df_combine %>% filter(!is.na(PubNum_CHNprop))
...
ggplot(df_NAclean, aes(x = year, y = PubNum_CHNprop, group = journal, color = journal)) + geom_line(size=2, alpha=0.5) + # geom_point() +
  scale_color_manual(values = cols) +
  theme_minimal() +
  labs(title = "From China", x = "Year", y = "Proportion") +
  scale_x_continuous(expand = c(0,0), limits = c(2000, 2031), breaks = seq(2000, 2024, by = 4), ) +
  # ylim(NA, 12000) +
  # add label of journal name
  # geom_text(data = df_NAclean %>% filter(year == 2024), aes(label = journal), hjust = +0.9, vjust = 0, size = 8) +
...
f1B <- ggplot(df_NAclean, aes(x = year, y = relativePrice, group = journal, color = journal)) + geom_line(size=2, alpha=0.5) + # geom_point() +
  ...
  scale_y_continuous(limits = c(NA, 5), breaks = seq(1, 5, by = 1), ) +
  # add label of journal name
  # geom_text(data = df_NAclean %>% filter(year == 2024), aes(label = journal), hjust = +0.9, vjust = 0, size = 8) +
  # change legend title
  labs(color = "") +
...
# Draw line graph for each journal, value on y, year on x
# cols <- kelly.colors(length(unique(df_inflation$Country)))
# cols <- glasbey.colors(length(unique(df_inflation$Country)))
# cols <- palette36.colors(length(unique(df_inflation$Country)))
# cols <- alphabet.colors(length(unique(df_inflation$Country)))
# cols <- dark.colors(length(unique(df_inflation$Country)))
# cols <- sky.colors(length(unique(df_inflation$Country)))
# cols <- green.armytage.colors(length(unique(df_inflation$Country)))
...
f2B <- ggplot(df_NAclean, aes(x = Year, y = research_calc, group = Country, color = Country)) + geom_line(size=2, alpha=0.5) + # geom_point() +
  scale_color_manual(values = cols) +
  theme_minimal() +
  labs(title = "Research & development spending", x = "Year", y = "USD") +
  scale_x_continuous(expand = c(0,0), limits = c(2000, 2031), breaks = seq(2000, 2024, by = 4), ) +
  #scale_y_continuous(limits = c(NA, 7), breaks = seq(1, 7, by = 1), ) +
  # add label of journal name
  # geom_text(data = df_NAclean %>% filter(year == 2024), aes(label = journal), hjust = +0.9, vjust = 0, size = 8) +
...
(f2A | plot_spacer()) / (f2B | f2C) / (f2D | f2E) +
...
# Remove journals
df_combine <- df_combine %>% filter(journal != "New England Journal of Medicine" &
                                      journal != "Lancet" &
                                      journal != "BMJ" &
                                      journal != "Nature" &
                                      journal != "Science" &
...
### CCF graphs ###
df_ccf <- read.csv("pub_fee_CCF.csv", header = TRUE, sep = ",")
